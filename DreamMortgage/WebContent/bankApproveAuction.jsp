	...
	$("#discountPercentage").blur(function(){
		...
		$('#submit').click(function(e) {
			$('#successMsg,#errorMsg').text("");
			 ...
			 if(isDiscountPercentageValid()&&isAtleastOneChecked){
				 $('#form').hide();
					document.getElementById('loading').style.display='block';
				 $('#bankForm').attr("action","bank?action=approvePurchase");
				   return true;
		...
		$('#closeAuction').click(function(e) {
			...
			if(!isAtleastOneChecked){
				...
				$('#form').hide();
				document.getElementById('loading').style.display='block';
			$('#bankForm').attr("action","bank?action=closeAuction&type=purchase");
			return true;
			}
...
<div class="sidebar">
  <ul>
    ...
    <li><a href="bank?action=getPurchaseRequest" class="sideActive">Approve Auction Transfers</a></li> 
...
<p id="desc" style="font-weight: bold; color: green;">On this page lending bank authorizes the transfer of disbursed mortgage deeds to Partner bank in return of capital.</p>
<article>
...
<input type="submit" id="closeAuction" value="Close Auction" style="width: 100px; margin-left: 40px; height:27px;">
